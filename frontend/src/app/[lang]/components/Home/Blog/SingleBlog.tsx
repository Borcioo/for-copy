import { SingleBlogProps } from "@/app/[lang]/components/Home/Blog/HomeBlog";
import Image from "next/image";
import React, { useState } from "react";
import { getStrapiMedia } from "@/app/[lang]/utils/api-helpers";

export default function SingleBlog({ data }: { data: SingleBlogProps }) {
  ...
  return (
    <div className="w-[358px] flex justify-center items-center flex-col gap-[24px] lg:mt-[18px]">
      ...
      <div className="flex text-black flex-col w-[279px] lg:w-[335px] h-full">
        <div className="w-full h-full flex flex-col justify-end items-start">
          {data.name && <p className="text-base font-bold">{data.name}</p>}
          {data.description && (
            <p className="text-[14px] lg:mt-2">{data.description}</p>
          )}
          <button
            onMouseEnter={() => {
              setIsHovered(true);
